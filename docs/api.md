...
* `type`: `body` の種別を `text` （テキスト・Markdown）か `html` （HTML）で指定。詳細は後述（デフォルトは `text`）
...
Shodoでは本文にHTMLを利用できます。HTMLを指定した場合ブロック要素内の本文を校正します。
インライン要素や属性の値は無視されますのでご注意ください。また校正の対象はHTML要素に含まれている必要があります。
...
* `<h1>` ～ `<h6>`: 見出し
* `<li>`: 一覧（`<li><p>...</p></li>` も一覧とみなされる）
* `<div>`、`<p>`、`<figcaption>` など: 本文
